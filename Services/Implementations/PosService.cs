...
namespace GamblersGrocery.Services.Implementations
{
    public class PosService : IPosService
    {
        ...
        public async Task<BillViewModel> ApplyDiscountAsync(BillViewModel bill, decimal extraDiscountPercent)
        {
            try
            {
                ...
                foreach (var item in bill.Items)
                {
                    ...
                    item.productDiscount = Math.Round(
                        (item.unitPrice ?? 0) * (item.quantity ?? 0) * item.discountPercent / 100, 2);

                    item.lineTotal = Math.Round(
                        ((item.unitPrice ?? 0) * (item.quantity ?? 0)) - item.productDiscount, 2);
                ...
                bill.billDiscountAmount = Math.Round(subTotal * extraDiscountPercent / 100, 2);
                bill.totalAmount = Math.Round(subTotal - bill.billDiscountAmount, 2);
        ...
        public async Task<Transaction> CompleteSaleAsync(
            BillViewModel bill,
            int cashierId,
            string cashierName,
            string paymentMode,
